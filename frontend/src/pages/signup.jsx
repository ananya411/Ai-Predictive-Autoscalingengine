            ...
            <div className="relative z-10 grid min-h-screen grid-cols-1 lg:grid-cols-2">
                ...
                <div className="hidden lg:flex flex-col justify-center px-20">
                    ...
                    <motion.ul initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} className="mt-10 space-y-4 text-white/70">
                        <li>⚡ Predict traffic spikes before they happen</li>
                        <li>📉 Reduce cloud costs with smart recommendations</li>
                        <li>🛡️ Maintain 99.99% uptime automatically</li>
                    </motion.ul>

                    {/* Animated stats */}
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="mt-12 grid grid-cols-3 gap-6">
                        <Stat value="32%" label="Cost Reduction" />
                        <Stat value="99.99%" label="Uptime" />
                        <Stat value="AI" label="Driven" />
                    </motion.div>

                    {/* Product visual */}
                    <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.8 }} className="relative mt-14 h-72 overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-[#9b87f5]/25 via-[#9b87f5]/10 to-transparent backdrop-blur-xl">
                        <img src="https://images.unsplash.com/photo-1558494949-ef010cbdcc31?auto=format&fit=crop&w=1200&q=80" alt="Cloud infrastructure dashboard" className="absolute inset-0 h-full w-full object-cover" />
                ...
                <div className="flex items-center justify-center px-6">
                    ...
                    >
                        ...
                        <div className="mb-8 text-center">
                            <h2 className="text-3xl font-bold bg-gradient-to-r from-white via-[#9b87f5] to-white bg-clip-text text-transparent">Create Account</h2>
                            <p className="mt-2 text-white/60">Start your CloudSense journey</p>
                        </div>

                        <form onSubmit={handleSubmit} className="space-y-5">
                            <div className="space-y-2">
                                <Label htmlFor="email" className="text-white/80">Email</Label>
                                <Input id="email" type="email" placeholder="name@company.com" value={formData.email} onChange={handleChange} required className="bg-white/5 border-white/10 text-white" />
                            </div>
                            <div className="space-y-2">
                                <Label htmlFor="password" className="text-white/80">Password</Label>
                                <Input id="password" type="password" placeholder="••••••••" value={formData.password} onChange={handleChange} required className="bg-white/5 border-white/10 text-white" />
                            </div>
                            <div className="space-y-2">
                                <Label htmlFor="confirmPassword" className="text-white/80">Confirm Password</Label>
                                <Input id="confirmPassword" type="password" placeholder="••••••••" value={formData.confirmPassword} onChange={handleChange} required className="bg-white/5 border-white/10 text-white" />
                            </div>
                        ...
                        <div className="mt-6 text-center text-sm text-white/60">
                            Already have an account? <Link to="/signin" className="text-[#9b87f5] hover:underline">Sign in</Link>
                        </div>
                    </motion.div>
                </div>
            </div>
